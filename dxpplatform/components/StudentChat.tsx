...
import {
  getStudentChatSession,
  sendStudentMessage,
} from "@/app/api/actions/studentChat";
...
function buildWelcome(context?: StudentChatContext) {
  return `Hi ${context?.userName ?? "there"}! I'm Thinkra, your project assistant for ${
    context?.projectTitle ?? "this project"
  }. Ask me anything.`;
}
...
export default function StudentChat({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const chatSession =
        await getStudentChatSession(
          context.projectId!
        ).catch((error) => {
          console.error(
            "Student chat session error:",
            error
          );
          return null;
        });
  ...
  }, [isOpen, context?.projectId]);

  // FIRST POSITION — bottom-right on first open
  useEffect(() => {
    if (!isOpen || initialised) return;
  ...
  const onDragStart = useCallback(
    (e: React.MouseEvent | React.TouchEvent) => {
      ...
      const move = (ev: MouseEvent | TouchEvent) => {
        ...
        const nx = Math.max(
          0,
          Math.min(cx - dragOffset.current.x, window.innerWidth - size.w)
        );
        const ny = Math.max(
          0,
          Math.min(
            cy - dragOffset.current.y,
  ...
  const send = async (text: string) => {
    if (!text.trim() || isLoading || !sessionId || !context?.projectId) return;
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  !input.trim() || isLoading ? "#a5b4fc" : "#4338ca",
                cursor:
